-- FRESH SETUP: Complete Database Schema & Seed
...
DROP TABLE IF EXISTS order_items CASCADE; -- Legacy table
...
CREATE TABLE products (
    ...
    image_url TEXT,
    category VARCHAR(50),
    sizes JSONB DEFAULT '[]'::jsonb, -- Store sizes as JSON array e.g. [39, 40, 41]
    color VARCHAR(50),
...
CREATE TABLE orders (
    ...
    items JSONB DEFAULT '[]'::jsonb, -- Store full order items as JSON for immutability
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    order_notes TEXT
);
...
CREATE TABLE admins (
    ...
    password_hash VARCHAR(255) NOT NULL, -- In production, use bcrypt. Here we store text for simplicity as per current auth implementation.
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE POLICY "Public read products" ON products FOR SELECT TO anon, authenticated, service_role USING (true);
-- Allow PUBLIC/ANON write access (Simplified for Dashboard without Service Role)
-- NOTE: In a strictly secure env, restricts this to authenticated users or service_role only.
CREATE POLICY "Admin manage products" ON products FOR ALL TO anon, authenticated, service_role USING (true);

-- Orders Policies
-- Allow PUBLIC insert (Checkout needs this)
CREATE POLICY "Public create orders" ON orders FOR INSERT TO anon, authenticated, service_role WITH CHECK (true);
-- Allow PUBLIC read (Success page needs this)
CREATE POLICY "Public read orders" ON orders FOR SELECT TO anon, authenticated, service_role USING (true);
-- Allow Admin manage
CREATE POLICY "Admin manage orders" ON orders FOR ALL TO anon, authenticated, service_role USING (true);

-- Admins Policies
-- Allow login check (Read)
CREATE POLICY "Public read admins" ON admins FOR SELECT TO anon, authenticated, service_role USING (true);
-- Allow self-management
CREATE POLICY "Admin manage self" ON admins FOR ALL TO anon, authenticated, service_role USING (true);
